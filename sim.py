...
Please use the new modular structure:
    ...
    - geometry.py  : Geometry computations
    - iq_gen.py    : IQ generation
    - estimation.py: Doppler estimators
    - kinematics.py: Kinematic recovery
    - plotting.py  : Plotting and animation
...
warnings.warn(
    "sim.py is deprecated. Please use the new modular structure: "
    "config, geometry, iq_gen, estimation, kinematics, plotting, main",
    DeprecationWarning,
    stacklevel=2,
)
...
from estimation import estimate_doppler_phase_diff, estimate_doppler_stft, _hann_smooth
...
def estimate_doppler_phase_diff(
    iq: np.ndarray, fs: float, smooth_window: int | None = None
) -> tuple:
    """
